...
/mob/living/proc/apply_damage(var/damage = 0,var/damagetype = BRUTE, var/def_zone = null, var/blocked = 0, var/damage_flags = 0, var/used_weapon = null)
	...
	switch(damagetype)
		...
		if(FATIGUE)
			adjustFatigueLoss(damage/(blocked+1))
	flash_weak_pain()
	updatehealth()
	return 1


/mob/living/proc/apply_damages(var/brute = 0, var/burn = 0, var/tox = 0, var/oxy = 0, var/clone = 0, var/halloss = 0, var/stamina = 0, var/fatigue = 0, var/def_zone = null, var/blocked = 0, var/damage_flags = 0)
	...
	if(fatigue) apply_damage(fatigue, FATIGUE, def_zone, blocked)
	return 1
