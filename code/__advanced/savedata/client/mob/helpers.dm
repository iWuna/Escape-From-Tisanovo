...
/savedata/client/mob/proc/create_new_character(var/character_id)
	...
	if(world_state != STATE_RUNNING)
		owner?.mob?.to_chat(user, "<span class=warning>The round is currently ending! Wait until next round!</span>")
		return FALSE

	if(text2num(character_id) > MAX_CHARACTERS)
		owner?.mob?.to_chat(user, "<span class=warning>You exceed the maximum allocated characters! ([text2num(character_id)-1]/[MAX_CHARACTERS])</span>")
		return FALSE
...
/savedata/client/mob/proc/save_character(var/mob/living/carbon/human/A,var/save_inventory = TRUE,var/force=FALSE,var/died=FALSE)

	if(!A)
		usr?.to_chat(user, "<span class=danger><h2>Save failed. Please contact the server owner with error code: 1000.</h2></span>")
		CRASH("FATAL ERROR: Tried to save a character without an actual character!")
		return FALSE

	if(!istype(A))
		usr?.to_chat(user, "<span class=danger><h2>Save failed. Tried to save [A.get_debug_name()]. Please contact the server owner with error code: 2000.</h2></span>")
		CRASH_SAFE("FATAL ERROR: Tried to save [A.get_debug_name()], a non-player!")
		return FALSE

	if(!force && world_state != STATE_RUNNING)
		usr?.to_chat(user, "<span class=danger>Your character was not saved as the round just ended!</span>")
		return FALSE

	if(!A.allow_save)
		usr?.to_chat(user, "<span class=danger>You cannot save this character!</span>")
		return FALSE

	if(!A.finalized || !A.initialized)
		usr?.to_chat(user, "<span class=warning>Your character was not saved as it is still initializing. This is to prevent save corruption. If you believe you received this message in error, contact Burger on discord.</span>")
		return FALSE

	var/list/loaded_data = A.get_mob_data(save_inventory,force,died)
	if(write_json_data_to_id(loaded_data["id"],loaded_data))
		if(died)
			A.to_chat(user, "<span class=notice>Your mind and body was backed up in the NanoTrasen cloning network...</span>")
		else
			A.to_chat(user, "<span class=notice>Sucessfully saved character [A.name].</span>")
	else
		A.to_chat(user, "<span class=danger><h2>Save failed. Please contact the server owner with error code: 99.</h2></span>")
